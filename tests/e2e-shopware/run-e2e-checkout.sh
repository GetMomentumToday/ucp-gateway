...
T13_UPDATE=$(curl -s -X PUT "$GATEWAY_URL/checkout-sessions/$T13_SID" \
  ...
  -d "{
    ...
    \"fulfillment\": {
      \"destinations\": [{\"id\": \"d1\", \"address\": {\"street_address\": \"789 Cancel St\", \"address_locality\": \"Berlin\", \"address_region\": \"BE\", \"postal_code\": \"10115\", \"address_country\": \"DE\"}}],
      \"methods\": [{\"id\": \"m1\", \"type\": \"shipping\", \"selected_destination_id\": \"d1\", \"groups\": [{\"id\": \"g1\", \"selected_option_id\": \"o1\", \"options\": [{\"id\": \"o1\", \"label\": \"Standard Shipping\", \"amount\": {\"value\": 499, \"currency\": \"EUR\"}}]}]}]
    }
  }")
...
PRODUCT_ID_2=$(curl -s "$GATEWAY_URL/ucp/products?q=Jacket" -H "$AGENT_HEADER" | python3 -c "
import sys,json
d=json.load(sys.stdin)
items = d if isinstance(d, list) else d.get('products', d.get('items', []))
if isinstance(items, list) and len(items) > 0:
...
T16_RESP=$(curl -s -X POST "$GATEWAY_URL/checkout-sessions" \
  -H "$AGENT_HEADER" -H "$CONTENT_TYPE" \
  -d "{\"line_items\": [{\"item\": {\"id\": \"$PRODUCT_ID\"}, \"quantity\": 1}, {\"item\": {\"id\": \"$PRODUCT_ID_2\"}, \"quantity\": 1}]}")
...
T17_UPD1=$(curl -s -X PUT "$GATEWAY_URL/checkout-sessions/$T17_SID" \
  ...
  -d "{
    ...
    \"fulfillment\": {
      \"destinations\": [{\"id\": \"d1\", \"address\": {\"street_address\": \"100 Totals St\", \"address_locality\": \"Berlin\", \"address_region\": \"BE\", \"postal_code\": \"10115\", \"address_country\": \"DE\"}}],
      \"methods\": [{\"id\": \"m1\", \"type\": \"shipping\", \"selected_destination_id\": \"d1\", \"groups\": [{\"id\": \"g1\", \"selected_option_id\": \"o1\", \"options\": [{\"id\": \"o1\", \"label\": \"Cheap Shipping\", \"amount\": {\"value\": 500, \"currency\": \"EUR\"}}]}]}]
    }
  }")
...
T17_UPD2=$(curl -s -X PUT "$GATEWAY_URL/checkout-sessions/$T17_SID" \
  ...
  -d "{
    ...
    \"fulfillment\": {
      \"destinations\": [{\"id\": \"d1\", \"address\": {\"street_address\": \"100 Totals St\", \"address_locality\": \"Berlin\", \"address_region\": \"BE\", \"postal_code\": \"10115\", \"address_country\": \"DE\"}}],
      \"methods\": [{\"id\": \"m1\", \"type\": \"shipping\", \"selected_destination_id\": \"d1\", \"groups\": [{\"id\": \"g1\", \"selected_option_id\": \"o2\", \"options\": [{\"id\": \"o2\", \"label\": \"Express Shipping\", \"amount\": {\"value\": 1500, \"currency\": \"EUR\"}}]}]}]
    }
  }")
...
if [ "$T17_TOTAL1" != "0" ] && [ "$T17_TOTAL2" != "0" ] && [ "$T17_TOTAL1" != "$T17_TOTAL2" ]; then
  TESTS=$((TESTS + 1)); PASS=$((PASS + 1))
  echo "  [PASS] Totals changed with different option ($T17_TOTAL1 → $T17_TOTAL2)"
else
  TESTS=$((TESTS + 1)); FAIL=$((FAIL + 1))
  echo "  [FAIL] Totals did not change ($T17_TOTAL1 → $T17_TOTAL2)"
fi
