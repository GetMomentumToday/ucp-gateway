/**
 * UCPM-226: E2E signing tests for Magento adapter.
 *
 * Runs against a live Magento instance + gateway.
...
const AGENT_HEADER = 'e2e-signing-magento/1.0';
...
describe('Magento E2E: Request Signing', () => {
  ...
  // ── Signing verification round-trip (simulated webhook) ─────────────
  ...
  describe('Checkout with Request-Signature header', () => {
    ...
    beforeAll(async () => {
      const searchResp = await get('/ucp/products?q=shoes&limit=1');
