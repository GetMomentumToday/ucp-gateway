...
async function testMagentoCheckout(): Promise<void> {
  ...
  const totals = await adapter.calculateTotals(cart.id, {
    shipping_address: {
      ...
      postal_code: '78701', country_iso2: 'US', region: 'TX',
    },
  });
  console.log(`  Totals: subtotal=$${(totals.subtotal_cents / 100).toFixed(2)}, shipping=$${(totals.shipping_cents / 100).toFixed(2)}, tax=$${(totals.tax_cents / 100).toFixed(2)}, total=$${(totals.total_cents / 100).toFixed(2)}`);
